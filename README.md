...
- 개인정보처리방침은 로그인 없이 `/privacy`에서 확인할 수 있습니다.
- 웹 계정 삭제는 로그인 상태와 관계없이 `/delete-account`에서 아이디와 현재 비밀번호로 본인 확인 후 요청할 수 있습니다.
- 로그인한 회원은 **정보수정 → 계정 삭제**에서도 동일한 하드 삭제를 실행할 수 있습니다.
- 길드장 계정은 다른 회원에게 길드장 역할을 이전한 뒤 삭제해야 합니다.

개인정보처리자는 김경일이며 문의 이메일은 `dev.kyungil@icloud.com`입니다. 현재 애플리케이션에는 자동 백업 절차가 없으며, 백업을 도입할 때는 개인정보처리방침의 보관·파기 내용도 함께 갱신해야 합니다.
