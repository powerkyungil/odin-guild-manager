...
assert.equal(JSON.stringify(utils.parseParticipationPasteData('축제\t그냥대지\t득꼬\n18%\t61%\t63%')), JSON.stringify({ mode: 'nickname', entries: [{ nickname: '축제', value: '18' }, { nickname: '그냥대지', value: '61' }, { nickname: '득꼬', value: '63' }] }));
assert.equal(JSON.stringify(utils.parseParticipationPasteData('축제 그냥대지 득꼬\n18% 61% 63%')), JSON.stringify({ mode: 'nickname', entries: [{ nickname: '축제', value: '18' }, { nickname: '그냥대지', value: '61' }, { nickname: '득꼬', value: '63' }] }));
assert.equal(JSON.stringify(utils.parseParticipationPasteData('| **축제** | **그냥대지** |\n| :----: | :------: |\n| 18% | 61% |')), JSON.stringify({ mode: 'nickname', entries: [{ nickname: '축제', value: '18' }, { nickname: '그냥대지', value: '61' }] }));
assert.equal(JSON.stringify(utils.mapParticipationPasteByNickname([{ nickname: '그냥대지', value: '61' }, { nickname: '없는회원', value: '20' }], [{ id: 2, nickname: '그냥대지' }])), JSON.stringify({ mappedEntries: [{ memberId: 2, nickname: '그냥대지', value: '61' }], unknownNicknames: ['없는회원'], duplicateNicknames: [], missingValueNicknames: [] }));
assert.equal(JSON.stringify(utils.mapParticipationPasteByNickname([{ nickname: ' 그냥\u200b대지 ', value: '61' }], [{ id: 2, nickname: '그냥대지' }])), JSON.stringify({ mappedEntries: [{ memberId: 2, nickname: '그냥대지', value: '61' }], unknownNicknames: [], duplicateNicknames: [], missingValueNicknames: [] }));
